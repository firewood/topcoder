...
public:
	long long count(vector <int> w) {
		...
		for (int i = 0; i != w.size(); ++i) {
			...
			LLLLMap::const_iterator q;
			ans += c[w[i]];
			for (q = bl.begin(); q != bl.end(); ++q) {
				LL d = (q->first.first * w[i]) / q->first.second;
				if (d > 0 && d <= 1000000000 && q->first.first * w[i] == d * q->first.second) {
					c[d] += q->second;
				}
			}
			for (LL x = 1; x < 1024; ++x) {
			...
			for (p = a.begin(); p != a.end(); ++p) {
				LL g = gcd(p->first, w[i]);
				LL x = p->first / g, y = w[i] / g;
				if (x < 1024 && y < 1024) {
					bs[x][y] += p->second;
				} else {
					bl[LLLL(x, y)] += p->second;
				}
//				b[LLLL(p->first / g, w[i] / g)] += p->second;
			}
			a[w[i]] += 1;
		}
